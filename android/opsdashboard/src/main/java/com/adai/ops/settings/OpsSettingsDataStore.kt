...
data class OpsSettings(
    ...
    // Relay mode (Cloudflare Tunnel via kerfbit.dev) — one toggle/token pair shared
    // across all three services, consistent with useSharedHost. Only changes scheme
    // and port handling; the user still types the relay subdomain into the existing
    // host field(s) above.
