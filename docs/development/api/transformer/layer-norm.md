...
- **`cached_mean`** (vector of float): Mean for each sample in the batch
- **`cached_var`** (vector of float): Variance for each sample in the batch
...
## LayerNorm Class Separation Details
