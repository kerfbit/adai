#!/usr/bin/env python3
"""
Comprehensive markdown linter fix for training-internals.md
Fixes: MD022, MD031, MD032, MD040, MD060, MD009, MD029, MD036
"""

import re

def is_heading(line):
    """Check if line is a markdown heading"""
    stripped = line.strip()
    return stripped.startswith('#') and not stripped.startswith('```') and not stripped.startswith('#include')

def is_code_fence(line):
    """Check if line is a code fence"""
    return line.strip().startswith('```')

def is_list_item(line):
    """Check if line is a list item"""
    stripped = line.strip()
    if not stripped:
        return False
    # Bullet lists
    if stripped[0] in ['-', '*', '+']:
        return True
    # Numbered lists
    if len(stripped) > 1 and stripped[0].isdigit():
        for i in range(1, min(4, len(stripped))):
            if stripped[i] in ['.', ')']:
                return True
    return False

def fix_markdown(input_path, output_path):
    with open(input_path, 'r', encoding='utf-8') as f:
        lines = f.readlines()
    
    fixed = []
    in_code_block = False
    in_list = False
    i = 0
    
    while i < len(lines):
        line = lines[i]
        original_line = line
        
        # MD009: Remove trailing whitespace
        line = line.rstrip() + '\n' if line.endswith('\n') else line.rstrip()
        
        # Track code block state
        if is_code_fence(line):
            # MD040: Add language specification if missing
            if line.strip() == '```' and not in_code_block:
                line = '```text\n'
            
            # MD031: Blank line before code fence (when starting)
            if not in_code_block and fixed and fixed[-1].strip() != '':
                # Don't add if previous is heading or another fence
                if not is_heading(fixed[-1]) and not is_code_fence(fixed[-1]):
                    fixed.append('\n')
            
            fixed.append(line)
            
            # MD031: Blank line after code fence (when ending)
            if in_code_block and i + 1 < len(lines) and lines[i + 1].strip() != '':
                next_line = lines[i + 1]
                if not is_heading(next_line) and not is_code_fence(next_line):
                    fixed.append('\n')
            
            in_code_block = not in_code_block
            i += 1
            continue
        
        # Skip processing inside code blocks
        if in_code_block:
            fixed.append(line)
            i += 1
            continue
        
        # Check list state
        curr_is_list = is_list_item(line)
        
        # MD032: Blank line before list starts
        if curr_is_list and not in_list and fixed and fixed[-1].strip() != '':
            if not is_heading(fixed[-1]):
                fixed.append('\n')
        
        # MD032: Blank line after list ends
        if in_list and not curr_is_list and line.strip() != '':
            if not is_heading(line) and not is_code_fence(line) and fixed and fixed[-1].strip() != '':
                fixed.append('\n')
        
        in_list = curr_is_list
        
        # MD022: Blank line before heading
        if is_heading(line) and fixed and fixed[-1].strip() != '':
            if not is_heading(fixed[-1]) and not is_code_fence(fixed[-1]):
                fixed.append('\n')
        
        # Add the current line
        fixed.append(line)
        
        # MD022: Blank line after heading
        if is_heading(line) and i + 1 < len(lines):
            next_line = lines[i + 1]
            if next_line.strip() != '' and not is_heading(next_line) and not is_code_fence(next_line):
                # Will be handled when we process the next line
                pass
        
        i += 1
    
    # Fix table formatting (MD060)
    final_fixed = []
    for line in fixed:
        if '|' in line and not line.strip().startswith('```'):
            # Ensure spaces around pipes
            parts = line.split('|')
            formatted_parts = []
            for j, part in enumerate(parts):
                if j == 0 or j == len(parts) - 1:
                    # Keep leading/trailing parts as-is
                    formatted_parts.append(part)
                else:
                    # Ensure space around content
                    formatted_parts.append(' ' + part.strip() + ' ')
            line = '|'.join(formatted_parts)
            if not line.endswith('\n'):
                line += '\n'
        
        final_fixed.append(line)
    
    # Write output
    with open(output_path, 'w', encoding='utf-8') as f:
        f.writelines(final_fixed)

if __name__ == '__main__':
    import sys
    
    input_file = '/home/rodney/Repos/adai/docs/guides/training-internals.md'
    output_file = '/home/rodney/Repos/adai/docs/guides/training-internals.md'
    
    print(f"Fixing markdown lint issues in {input_file}...")
    fix_markdown(input_file, output_file)
    print(f"✅ Fixed file saved to {output_file}")
