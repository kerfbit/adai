...
#
# Usage:
#   sudo ./install_registry_server.sh [OPTIONS]
#
...
Options:
  ...
  --port PORT           Listening port for registry_server (default: 8082)
  --data-dir DIR        Root directory for per-group registry state
                        (default: <install-path>/registry_sessions)
...
Examples:
  ...
  sudo $0 --install-path /usr/local/adai --port 9082
...
Description:
  Installs registry_server (ADAI Distributed Dataset Registry daemon) to a local
  host. Creates the required directory layout, copies the binary, sets
  permissions and ownership, writes and enables a systemd service unit.

  The registry daemon coordinates the shared pending-file queue and data
  registry for multi-machine training pools (TD-028 Phase 9). Each per-group
  namespace's state lives under the data directory:
  data_registry.txt, pending_files.txt, uploads/, datasets/, fetch_cursors/,
  plus this daemon's own daemon_config.db (admin-mutable settings) and
  session_counters.db (run/session numbering — see CLAUDE.md "Configuration").

  This installs registry_server with its embedded FTP data server disabled
  (matches registry_server's own default). To enable FTP dataset transport,
  edit the systemd unit's ExecStart after installing (see
  "registry_server --help" for --ftp-enabled and related flags) or install via
  scripts/install_server_bundle.sh, which shares one config surface across all
  three daemons.

  Binary source: <build-dir>/bin/registry_server
  Install target: <install-path>/bin/registry_server
  Data directory: <install-path>/registry_sessions/
  Service name:   adai-registry
  Default port:   8082

  Build first:
    cmake --preset portable && cmake --build --preset portable --target registry_server
...
DATA_DIR="${DATA_DIR_OVERRIDE:-${INSTALL_PATH}/registry_sessions}"
LOG_DIR="/var/log/adai"

BUILD_BIN_DIR="${REPO_ROOT}/${BUILD_DIR}/bin"

SERVICE_NAME="adai-registry"
...
preflight_checks() {
    ...
    if [[ ! -f "${BUILD_BIN_DIR}/registry_server" ]]; then
        ...
        error "Build with:"
        error "  cmake --preset portable && cmake --build --preset portable --target registry_server"
        exit 1
    fi
...
install_registry_server() {
    ...
    echo "  Listen Port:    ${REGISTRY_PORT}"
    echo "  Build Source:   ${BUILD_BIN_DIR}/registry_server"
    ...
    # Step 3: Copy binary
    info "[3/${step_total}] Installing registry_server binary..."
    cp "${BUILD_BIN_DIR}/registry_server" "${BIN_DIR}/registry_server"
    chmod 755 "${BIN_DIR}/registry_server"
    success "Installed ${BIN_DIR}/registry_server"
...
[Unit]
Description=ADAI Distributed Dataset Registry
...
EOF
    ...
    if systemctl start "${SERVICE_NAME}.service"; then
        success "registry_server started"
    else
        error "Failed to start registry_server"
...
print_summary() {
    echo ""
    echo "========================================================================"
    success "ADAI Distributed Dataset Registry installed!"
    echo "========================================================================"
    echo ""
    echo "Installed:"
    echo "  Binary:   ${BIN_DIR}/registry_server"
    ...
    echo ""
    echo "API endpoints (port ${REGISTRY_PORT}):"
    echo "  GET  http://localhost:${REGISTRY_PORT}/health"
    echo "  GET  http://localhost:${REGISTRY_PORT}/registry/<group>/queue"
    echo ""
    echo "Configure clients with:"
    echo "  REGISTRY_SERVER_URL=http://$(hostname -f 2>/dev/null || hostname):${REGISTRY_PORT}"
    echo ""
    echo "Override environment variables or add FTP transport flags:"
    echo "  sudo systemctl edit ${SERVICE_NAME}"
    echo "  (see 'registry_server --help' for --ftp-enabled and related flags)"
    echo ""
    echo "========================================================================"
    echo ""
}
...
echo ""
info "ADAI Distributed Dataset Registry — Installation Script"
echo ""
