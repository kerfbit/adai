...
class VocabValidator:
    ...
    def validate(self) -> bool:
        ...
        # VOCAB_SIZE should equal the total number of unique token IDs (max_id + 1)
        actual_vocab_count = len(self.vocab_entries)
        if self.vocab_size:
            special_token_ids = set(self.special_tokens.values())
            all_vocab_ids = [entry[1] for entry in self.vocab_entries]
            all_ids = list(special_token_ids) + all_vocab_ids
            
            if all_ids:
                max_id = max(all_ids)
                required_size = max_id + 1  # 0-indexed, so add 1
                if self.vocab_size != required_size:
                    self.warnings.append(f"VOCAB_SIZE mismatch: declared={self.vocab_size}, required={required_size} (max_id={max_id})")
        ...
        if all_ids:
            ...
            max_special_id = max(special_token_ids) if special_token_ids else -1
            expected_start = max_special_id + 1
            expected_end = expected_start + len(token_ids) - 1
            if token_ids and (min(token_ids) != expected_start or max(token_ids) != expected_end):
                self.warnings.append(f"Non-sequential IDs detected: expected {expected_start}-{expected_end}, got {min(token_ids)}-{max(token_ids)}")
    ...
    def repair(self, compact_and_reorder: bool = True) -> bool:
        ...
        if compact_and_reorder:
            # Sort entries by token ID
            self.vocab_entries.sort(key=lambda x: x[1])
            self.repairs_made.append("Sorted vocab entries by token ID")
            
            # Find the starting ID for regular vocab (after special tokens)
            special_token_ids = set(self.special_tokens.values())
            max_special_id = max(special_token_ids) if special_token_ids else -1
            start_id = max_special_id + 1
            
            # Check for ID range issues or gaps
            current_ids = [entry[1] for entry in self.vocab_entries]
            needs_compacting = False
            
            # Check if any IDs exceed reasonable range or have gaps
            if current_ids:
                max_id = max(current_ids)
                min_id = min(current_ids)
                expected_max = start_id + len(self.vocab_entries) - 1
                
                if max_id != expected_max or min_id < start_id:
                    needs_compacting = True
                    self.repairs_made.append(f"Detected ID range issues: min={min_id}, max={max_id}, expected={start_id}-{expected_max}")
            
            # Compact and reassign IDs sequentially
            if needs_compacting:
                compacted_entries = []
                new_id = start_id
                id_mapping = {}  # old_id -> new_id
                
                for token, old_id in self.vocab_entries:
                    id_mapping[old_id] = new_id
                    compacted_entries.append((token, new_id))
                    new_id += 1
                
                self.vocab_entries = compacted_entries
                self.repairs_made.append(f"Compacted and reassigned IDs: {start_id} to {new_id - 1} ({len(self.vocab_entries)} tokens)")
        
        # Update VOCAB_SIZE to match total count (special tokens + vocab entries)
        # VOCAB_SIZE should be large enough to accommodate all token IDs
        special_token_ids = set(self.special_tokens.values())
        all_vocab_ids = [entry[1] for entry in self.vocab_entries]
        all_ids = list(special_token_ids) + all_vocab_ids
        ...
        if all_ids:
            max_id = max(all_ids)
            # VOCAB_SIZE should be at least max_id + 1 to accommodate all IDs (0-indexed)
            required_vocab_size = max_id + 1
            
            if self.vocab_size != required_vocab_size:
                old_size = self.vocab_size
                self.vocab_size = required_vocab_size
                self.repairs_made.append(f"Updated VOCAB_SIZE from {old_size} to {required_vocab_size} (max_id={max_id})")
...
def main():
    ...
    parser.add_argument('--no-compact', action='store_true', help='Skip compacting and reordering IDs')
    ...
    if args.repair and not args.test_only:
        if not is_valid or validator.warnings:
            ...
            validator.repair(compact_and_reorder=compact)
