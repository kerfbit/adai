...
ChatbotAPI::ChatbotAPI(EncoderDecoderModel* model, BPETokenizer* tokenizer, int port,
    ...
    server_impl_->server.Post(
        "/chat/session", [this](const httplib::Request& req, httplib::Response& res) {
            try {
    ...
    server_impl_->server.Post(
        "/chat/batch", [this](const httplib::Request& req, httplib::Response& res) {
            try {
    ...
    server_impl_->server.Post(
        "/chat/batch-session", [this](const httplib::Request& req, httplib::Response& res) {
            try {
...
std::vector<std::string> ChatbotAPI::parse_json_array(const std::string& json,
    ...
    while (pos < array_end) {
        ...
        if (json[pos] == '"') {
            ...
            while (value_end < array_end) {
                if (json[value_end] == '"' &&
                    (value_end == value_start || json[value_end - 1] != '\\')) {
                    break;
                }
                value_end++;
            }
...
ChatbotAPI::BatchResponse ChatbotAPI::generate_batch_responses(
    const std::vector<std::string>& inputs, const GenerationConfig& config) {
    ...
    try {
        ...
        for (const auto& input : inputs) {
            input_token_sequences.push_back(
...
ChatbotAPI::BatchResponse ChatbotAPI::generate_batch_session_responses(
    const std::vector<std::string>& inputs, const std::vector<std::string>& session_ids,
    const GenerationConfig& config) {
    BatchResponse batch_response;
    batch_response.success = true;
