...
ChatbotCLI::ChatbotCLI(const std::string& server_url, const std::string& conv_save_file)
        ...
        if (!session_id.empty()) {
            ...
            if (res && res->status == 200) {
                std::cout << COLOR_SYSTEM << "✅ Conversation history cleared (Session "
                          << session_id << ")" << COLOR_RESET << std::endl;
            } else {
...
void ChatbotCLI::handle_setting(std::string_view setting) {
    ...
    if (param == "strategy") {
        ...
        std::cout << COLOR_ERROR << "❌ Unknown parameter: " << param << COLOR_RESET << std::endl;
    }
}
