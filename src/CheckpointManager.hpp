...
struct CheckpointInfo {
    ...
    CheckpointInfo(const std::string& path, int ep, float train_l, float val_l, std::time_t ts,
                   bool best = false)
        : filepath(path),
          epoch(ep),
          train_loss(train_l),
...
class CheckpointManager {
    ...
    void rotate_checkpoints() {
        ...
        std::sort(checkpoints_.begin(), checkpoints_.end(),
                  [](const CheckpointInfo& a, const CheckpointInfo& b) {
                      ...
                      if (a.validation_loss == 0.0f)
                          return false;
                      if (b.validation_loss == 0.0f)
                          return true;
   ...
   public:
     ...
     */
    CheckpointManager(const std::string& checkpoint_dir = "checkpoints/", int max_checkpoints = 5)
        ...
        try {
            ...
            std::cerr << "Warning: Could not create checkpoint directory: " << e.what()
                      << std::endl;
        }
    ...
    void load_existing_checkpoints() {
        ...
        try {
            ...
            for (const auto& entry : std::filesystem::directory_iterator(checkpoint_dir_)) {
                if (entry.is_regular_file() && entry.path().extension() == ".bin" &&
                    ...
                    if (info.epoch > 0 || info.train_loss > 0.0f) {  // Valid metadata
                        ...
                        if (info.is_best || (info.validation_loss > 0.0f &&
            ...
            std::cerr << "Warning: Error loading existing checkpoints: " << e.what() << std::endl;
        }
    ...
    std::string save_checkpoint(int epoch, float train_loss, float validation_loss = 0.0f) {
        ...
        CheckpointInfo info(filepath, epoch, train_loss, validation_loss, std::time(nullptr),
                            is_best);
    ...
    void print_summary() const {
        ...
        if (!checkpoints_.empty()) {
            ...
            for (size_t i = start; i < checkpoints_.size(); ++i) {
                ...
                std::cout << "    Epoch " << ckpt.epoch << " - Train: " << ckpt.train_loss;
