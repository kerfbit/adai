...
public:
    ...
    public:
        BatchIterator(const Dataset* dataset, const std::vector<size_t>* indices, 
        ...
        BatchIterator(const Dataset* dataset, std::shared_ptr<std::vector<size_t>> indices, 
                     size_t batch_size, size_t pos = 0)
            : dataset_(dataset), owned_indices_(indices), 
              batch_size_(batch_size), current_pos_(pos) {
    ...
    bool parse_json_format(std::ifstream& file) {
        ...
        while (std::getline(file, line)) {
            ...
            if (input_pos != std::string::npos && target_pos != std::string::npos) {
                ...
                std::string input = line.substr(input_quote_start, input_quote_end - input_quote_start);
                ...
                std::string target = line.substr(target_quote_start, target_quote_end - target_quote_start);
...
public:
    ...
    Dataset(unsigned int seed = 42) 
        : is_split_(false), num_folds_(0), rng_(seed), 
          augmentation_fn_(nullptr), preprocess_fn_(nullptr) {}
    ...
    public:
        ...
        BatchIterator begin() const { return begin_; }
        BatchIterator end() const { return end_; }
    };
     ...
     */
    void split_stratified(float train_ratio = 0.8f, float val_ratio = 0.1f, 
                         float test_ratio = 0.1f, int num_bins = 5) {
        if (data_.empty()) {
            std::cerr << "Warning: Cannot split empty dataset" << std::endl;
            return;
        }
        ...
        for (size_t i = 0; i < data_.size(); ++i) {
            ...
            int bin = std::min(static_cast<int>(total_len * num_bins / (max_len + 1)), num_bins - 1);
        ...
        for (auto& bin : bins) {
            ...
            validation_indices_.insert(validation_indices_.end(), 
                                      bin.begin() + train_end, bin.begin() + val_end);
    ...
    void get_fold(int fold_idx, std::vector<DataSample>& train_data, 
        ...
        for (int i = 0; i < num_folds_; ++i) {
            if (i != fold_idx) train_size += fold_indices_[i].size();
        }
     ...
     */
    int get_num_folds() const { return num_folds_; }
    ...
    void filter_by_length(size_t min_length, size_t max_length) {
        auto new_end = std::remove_if(data_.begin(), data_.end(),
            [min_length, max_length](const DataSample& sample) {
    ...
    void filter_by_pattern(const std::string& pattern, bool keep_matching = true) {
        auto new_end = std::remove_if(data_.begin(), data_.end(),
            [&pattern, keep_matching](const DataSample& sample) {
    ...
    void lowercase() {
        for (auto& sample : data_) {
            std::transform(sample.input.begin(), sample.input.end(), 
                         sample.input.begin(), ::tolower);
            std::transform(sample.target.begin(), sample.target.end(), 
