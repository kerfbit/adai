...
class EfficientBatching {
   public:
    ...
    static std::vector<SequenceBatch> create_dynamic_batches(
        const std::vector<std::vector<int>>& sequences, size_t batch_size,
        int pad_token_id = adai::SpecialTokenIDs::PAD,
        PaddingStrategy strategy = PaddingStrategy::RIGHT, bool sort_by_length = true) {
        if (sequences.empty()) {
        ...
        for (size_t i = 0; i < indices.size(); i += batch_size) {
            ...
            SequenceBatch batch =
                create_single_batch(sequences, batch_indices, pad_token_id, strategy);
    ...
    static std::vector<SequenceBatch> create_bucketed_batches(
        const std::vector<std::vector<int>>& sequences, const BucketConfig& config,
        int pad_token_id = adai::SpecialTokenIDs::PAD,
     ...
     */
    static void apply_augmentation(std::vector<std::vector<int>>& sequences,
...
#pragma omp for schedule(dynamic, 16)
            for (size_t seq_idx = 0; seq_idx < sequences.size(); ++seq_idx) {
                ...
                if (config.enable_sequence_shuffle && config.shuffle_prob > 0.0f &&
                    seq.size() > 1) {
    ...
    static BatchStatistics calculate_statistics(const std::vector<SequenceBatch>& batches) {
        ...
        stats.avg_sequence_length =
            total_sequences > 0
                ? static_cast<double>(total_tokens - total_padding) / total_sequences
                : 0.0;
        stats.padding_ratio =
            total_tokens > 0 ? static_cast<double>(total_padding) / total_tokens : 0.0;
     ...
     */
    static std::vector<int> pad_sequence(const std::vector<int>& sequence, int target_length,
                                         int pad_token_id, PaddingStrategy strategy) {
     ...
     */
    static std::vector<int> create_attention_mask(int original_length, int padded_length,
   ...
   private:
     ...
     */
    static SequenceBatch create_single_batch(const std::vector<std::vector<int>>& sequences,
                                             const std::vector<size_t>& indices, int pad_token_id,
                                             PaddingStrategy strategy) {
        SequenceBatch batch;
        ...
        for (size_t idx : indices) {
            batch.max_length = std::max(batch.max_length, static_cast<int>(sequences[idx].size()));
        }
        ...
        for (size_t idx : indices) {
            ...
            // Pad sequence
            std::vector<int> padded = pad_sequence(seq, batch.max_length, pad_token_id, strategy);
            batch.sequences.push_back(padded);

            // Create attention mask
            std::vector<int> mask =
                create_attention_mask(original_length, batch.max_length, strategy);
