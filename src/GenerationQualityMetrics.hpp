...
/**
 * @brief Lightweight, header-only BLEU and ROUGE evaluator.
 *
...
private:
     ...
     * Corpus-level BLEU-N.
     *
     * Modified precision for each example is clipped so that each reference
     * n-gram cannot be matched more times than it appears.  Corpus-level
     * totals are accumulated before dividing to match the standard definition.
    ...
    /**
     * Macro-averaged corpus ROUGE-N F1.
     *
    ...
    /**
     * Macro-averaged corpus ROUGE-L F1 (based on Longest Common Subsequence).
     *
    ...
    /// LCS length via rolling 2-row DP — O(m·n) time, O(n) space.
