...
private:
        ...
        while (!shutdown_.load()) {
            ...
            if (should_emit && !pending_requests.empty()) {
                ...
                for (const auto& req : batch.requests) {
                    batch.tokenized_inputs.push_back(
                        tokenizer_->encode(req.input_text)
                    );
                }
    ...
    void decoder_worker() {
        while (!shutdown_.load()) {
            ...
            double decoder_time_ms = std::chrono::duration<double, std::milli>(
     ...
     */
    std::string generate_from_encoder_output(const Matrix& encoder_output,
                                             int max_length,
...
public:
     ...
     * Constructor
     */
    IntegratedInferenceEngine(LLMEncoder* encoder,
                             LLMDecoder* decoder,
     ...
     */
    std::future<std::string> submit(const std::string& input_text,
                                    int max_length = -1,
                                    const std::string& strategy = "") {
        if (max_length < 0) {
    ...
    IntegratedInferenceStats get_stats() const {
        ...
        if (stats_copy.avg_encoder_time_ms > 0 && stats_copy.avg_decoder_time_ms > 0) {
            double total_sequential = stats_copy.avg_encoder_time_ms + stats_copy.avg_decoder_time_ms;
            double pipeline_time = std::max(stats_copy.avg_encoder_time_ms, stats_copy.avg_decoder_time_ms);
