...
public:
    ...
    const Matrix& get_B() const { return B_; }
};
...
inline int calculate_lora_params_attention(int d_model, const LoRAConfig& config) {
    ...
    if (config.apply_to_query) num_projections++;
    if (config.apply_to_key) num_projections++;
    if (config.apply_to_value) num_projections++;
    if (config.apply_to_output) num_projections++;
