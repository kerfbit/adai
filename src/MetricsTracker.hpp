   ...
   public:
    ...
    void record_epoch(int epoch, float train_loss, float validation_loss = 0.0f,
        ...
        metrics.validation_perplexity =
            validation_loss > 0.0f ? calculate_perplexity(validation_loss) : 0.0f;
    ...
    bool export_csv(const std::string& filepath) const {
        ...
        // Write data
        for (const auto& metrics : history_) {
            file << metrics.epoch << "," << metrics.train_loss << "," << metrics.validation_loss
                 << "," << metrics.train_perplexity << "," << metrics.validation_perplexity << ","
                 << metrics.learning_rate << "," << metrics.gradient_norm << ","
    ...
    void print_summary() const {
        ...
        std::cout << "\n  Best Metrics:\n";
        std::cout << "    Best Train Loss: " << best_train_loss_ << " (Epoch " << best_train_epoch_
                  << ")\n";

        if (best_validation_loss_ < std::numeric_limits<float>::max()) {
            std::cout << "    Best Val Loss: " << best_validation_loss_ << " (Epoch "
                      << best_validation_epoch_ << ")\n";
        }
    ...
    void print_history() const {
        ...
        std::cout << "╚══════════════════════════════════════════════════════════╝\n";
        std::cout << std::fixed << std::setprecision(4);
        std::cout << std::setw(6) << "Epoch" << std::setw(12) << "Train Loss" << std::setw(12)
                  << "Val Loss" << std::setw(12) << "Train PPL" << std::setw(12) << "Val PPL"
                  << std::setw(10) << "LR" << std::setw(10) << "Grad\n";
        std::cout << std::string(74, '-') << "\n";

        for (const auto& metrics : history_) {
            std::cout << std::setw(6) << metrics.epoch << std::setw(12) << metrics.train_loss
                      << std::setw(12)
                      << (metrics.validation_loss > 0.0f ? std::to_string(metrics.validation_loss)
                                                         : "N/A")
                      << std::setw(12) << metrics.train_perplexity << std::setw(12)
                      << (metrics.validation_perplexity > 0.0f
                              ? std::to_string(metrics.validation_perplexity)
                              : "N/A")
                      << std::setw(10) << metrics.learning_rate << std::setw(10)
