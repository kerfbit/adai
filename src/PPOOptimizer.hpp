...
struct Trajectory {
    ...
    void add_step(const std::vector<float>& state, int action, 
                  float reward, float log_prob, float value) {
    ...
    size_t length() const { return states.size(); }
...
private:
    ...
    float relu_derivative(float x) const { return x > 0.0f ? 1.0f : 0.0f; }
...
private:
    ...
    std::vector<float> compute_gae(const Trajectory& traj) {
        ...
        for (float adv : advantages) mean += adv;
        mean /= advantages.size();
        for (float adv : advantages) var += (adv - mean) * (adv - mean);
        var /= advantages.size();
    ...
    float compute_policy_loss(float ratio, float advantage) {
        float clipped_ratio = std::max(1.0f - config_.clip_epsilon,
                                       std::min(1.0f + config_.clip_epsilon, ratio));
...
public:
    ...
    PPOOptimizer(RewardModel* reward_model, 
        ...
        : config_(config),
          ...
          value_function_(state_dim, {256, 128, 1}) {}
    ...
    float update(Trajectory& trajectory) {
        ...
        for (int epoch = 0; epoch < config_.num_epochs; epoch++) {
            ...
            for (size_t start = 0; start < trajectory.length(); start += config_.batch_size) {
                ...
                float value_loss = value_function_.update(batch_states, 
                                                          batch_returns,
     ...
     */
    const PPOConfig& get_config() const { return config_; }
    
    /**
     * @brief Update configuration
     */
    void set_config(const PPOConfig& config) { config_ = config; }
};
