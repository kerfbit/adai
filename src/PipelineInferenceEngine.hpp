    ...
    void encoder_worker() {
        while (running_) {
            ...
            try {
                ...
                double stage_time = std::chrono::duration<double, std::milli>(
                ...
                // Update statistics
                {
                    ...
                    stats_.avg_encoder_time_ms = 
                        total_encoder_time_ms_ / stats_.encoder_processed;
                }
        ...
        while (running_) {
            ...
            try {
                ...
                double total_latency = std::chrono::duration<double, std::milli>(
                    stage_end - enc_output.submit_time).count();
                ...
                // Update statistics
                {
                    ...
                    stats_.avg_decoder_time_ms = 
                        total_decoder_time_ms_ / stats_.decoder_processed;
                    stats_.avg_total_latency_ms = 
                        total_latency_ms_ / stats_.decoder_processed;
...
public:
     ...
     */
    std::future<std::vector<std::string>> submit_batch(
    ...
    std::future<std::string> submit(const std::string& input_text, int max_length = 100) {
        ...
        return std::async(std::launch::deferred, [](std::future<std::vector<std::string>> f) {
            auto results = f.get();
...
using StandardPipelineEngine = PipelineInferenceEngine<LLMEncoder, LLMDecoder, LanguageModelHead, BPETokenizer>;
