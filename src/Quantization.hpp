...
struct QuantizationParams {
    ...
    QuantizationParams()
        : scale(1.0f), zero_point(0), qmin(-127), qmax(127) {}
...
public:
     ...
     */
    Matrix dequantize_matrix(const std::vector<int8_t>& quantized,
                             int rows, int cols,
                             const QuantizationParams& params) const {
 ...
 */
inline void print_quantization_stats(const Matrix& original,
