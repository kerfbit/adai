...
public:
    ...
    void addKnowledgeBase() {
        ...
        std::vector<std::pair<std::string, std::string>> docs = {
            {"rag_concept", "RAG (Retrieval-Augmented Generation) combines retrieval with generation. "
            ...
            {"bert_concept", "BERT (Bidirectional Encoder Representations from Transformers) is an "
            ...
            {"transformer", "Transformers use self-attention mechanisms. They process sequences in "
                          "parallel unlike RNNs. Key components: multi-head attention, feedforward "
                          "networks, positional encoding. Used in BERT, GPT, T5."},
            
            {"embeddings", "Embeddings are dense vector representations of text. BERT produces "
            ...
            {"generation", "Text generation is producing coherent text autoregressively. Decoder models "
        ...
        std::cout << GREEN << "Knowledge base ready with " << rag->getNumDocuments() 
                  << " documents" << RESET << "\n";
    }
    ...
    void testRetrievalQuality() {
        ...
        result.details = std::to_string(correct) + "/" + std::to_string(test_queries.size()) + " correct";
    ...
    void testKnowledgeUpdate() {
        ...
        std::string new_doc_text = "GPT-4 is a large language model released in 2023. "
        ...
        if (!before.empty()) {
            std::cout << "  Retrieved: " << before[0].second->id 
                     << " (score: " << before[0].first << ")\n";
        } else {
        ...
        if (!after.empty()) {
            std::cout << "  Retrieved: " << after[0].second->id 
                     << " (score: " << after[0].first << ")\n";
        }
        ...
        result.details = success ? "New knowledge immediately accessible" : "Failed to retrieve new document";
        result.time_ms = time_ms;
        results.push_back(result);
    ...
    void testPerformance() {
        ...
        double encoding_ms = std::chrono::duration<double, std::milli>(end_encoding - start_encoding).count();
        double avg_encoding = encoding_ms / num_iterations;

        std::cout << "Average times (" << num_iterations << " iterations):\n";
        std::cout << "  Encoder embedding: " << std::fixed << std::setprecision(2) 
                  << avg_encoding << " ms\n";
        ...
        result.passed = true;
        result.details = "Retrieval adds " + std::to_string(avg_retrieval - avg_encoding) + " ms overhead";
        result.time_ms = avg_retrieval;
        results.push_back(result);

        std::cout << "\n" << YELLOW << "Note: RAG has retrieval latency but gains factual accuracy" 
                  << RESET << "\n";
    }
    ...
    void printSummary() {
        ...
        std::cout << "\n" << BOLD << "Overall: " << passed << "/" << results.size() 
                  << " tests passed" << RESET << "\n";
