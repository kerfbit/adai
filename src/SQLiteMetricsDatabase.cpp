...
void SQLiteMetricsDatabase::bootstrap_schema() {
    ...
    if (count == 0) {
        ...
        std::string insert = "INSERT INTO schema_version (version, applied_at) VALUES (1, '" + now + "');";
...
std::chrono::system_clock::time_point SQLiteMetricsDatabase::parse_timestamp(
    const std::string& s) {
    if (s.empty()) return {};
...
void SQLiteMetricsDatabase::insert_metrics_record(
...
void SQLiteMetricsDatabase::insert_abnormal_sample(
    const std::string& session_key,
...
void SQLiteMetricsDatabase::insert_generation_quality(
    const std::string& session_key,
    int epoch,
...
std::vector<PersistentMetricsRecord> SQLiteMetricsDatabase::query_history(
    ...
    sqlite3_stmt* stmt = nullptr;
    check_sqlite(sqlite3_prepare_v2(db_, sql.c_str(), -1, &stmt, nullptr), db_, "query_history prepare");

    int bind_idx = 1;
    sqlite3_bind_text(stmt, bind_idx++, session_key.c_str(), -1, SQLITE_TRANSIENT);
    if (from) sqlite3_bind_text(stmt, bind_idx++, from_str.c_str(), -1, SQLITE_TRANSIENT);
    if (to) sqlite3_bind_text(stmt, bind_idx++, to_str.c_str(), -1, SQLITE_TRANSIENT);
...
std::vector<SessionRecord> SQLiteMetricsDatabase::list_sessions(
    ...
    sqlite3_stmt* stmt = nullptr;
    check_sqlite(sqlite3_prepare_v2(db_, sql.c_str(), -1, &stmt, nullptr), db_, "list_sessions prepare");
...
std::unique_ptr<IMetricsDatabase> MetricsDatabaseFactory::create(
    ...
    adai::Logger::warn("[MetricsDatabaseFactory] Unknown backend '{}'; falling back to file-only", backend);
    return nullptr;
}
