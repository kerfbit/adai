...
private:
     ...
     */
    int verify_candidates(const std::vector<int>& context,
                          std::vector<TokenProposal>& proposals) {
        ...
        for (size_t i = 0; i < proposals.size(); i++) {
            ...
            std::vector<float> target_probs = target_generator_->get_next_token_probs(extended_context);
...
public:
     ...
     */
    SpeculativeDecoder(TextGenerator* draft_generator,
                      TextGenerator* target_generator,
    ...
    std::string generate(const std::string& prompt) {
        ...
        while (output_tokens.size() < static_cast<size_t>(config_.max_length)) {
            ...
            current_context.insert(current_context.end(), 
                                  output_tokens.begin(), 
                                  output_tokens.end());
    ...
    float get_acceptance_rate() const {
        if (total_proposals_ == 0) return 0.0f;
     ...
     */
    const SpeculativeDecodingConfig& get_config() const { return config_; }
...
inline float calculate_theoretical_speedup(int num_candidates, float acceptance_rate) {
    if (num_candidates <= 0 || acceptance_rate <= 0.0f) return 1.0f;
