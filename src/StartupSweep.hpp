...
inline void startup_sweep(DatasetRegistry& reg,
                           const std::string& run_id,
                           const std::string& download_dir) {
    namespace fs = std::filesystem;

    if (download_dir.empty() || !fs::exists(download_dir)) return;
    ...
    for (const auto& entry : fs::directory_iterator(download_dir)) {
        ...
        if (trained_filenames.count(filename)) {
            adai::Logger::info("[CLEANUP-D] Found locally retained file '{}' already marked "
                               "trained in registry. Deleting local copy.", filename);
        ...
        if (it == assigned_by_filename.end()) {
            ...
            adai::Logger::info("[CLEANUP-G] Found orphaned file '{}' in download_dir with "
        ...
        if (file_size == 0) {
            ...
            adai::Logger::info("[CLEANUP-A] Found empty partial download '{}' in download_dir. "
                               "Token from previous session expired — releasing assignment for "
                               "run_id '{}' and deleting file.", filename, run_id);
            fs::remove(local_path);
        ...
        adai::Logger::info("[CLEANUP-B/C] Found local file '{}' ({} bytes) assigned to "
                           "run_id '{}'. Keeping — DataTransport will verify size and "
                           "skip or resume download.", filename, file_size, run_id);
    }
}
