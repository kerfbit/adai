...
inline float json_flt(const std::string& json, const std::string& key, float def) {
    ...
    size_t vpos = colon + 1;
    while (vpos < json.size() && std::isspace(static_cast<unsigned char>(json[vpos]))) ++vpos;
    if (vpos >= json.size()) return def;
    try { return std::stof(json.substr(vpos)); } catch (...) { return def; }
}
...
inline int json_inum(const std::string& json, const std::string& key, int def) {
    ...
    while (vpos < json.size() && std::isspace(static_cast<unsigned char>(json[vpos]))) ++vpos;
    if (vpos >= json.size()) return def;
    try { return std::stoi(json.substr(vpos)); } catch (...) { return def; }
}
...
inline std::string sample_to_jsonl(const std::string& input, const std::string& response,
    ...
    std::ostringstream o;
    o << "{\"input\":\""      << json_esc(input)
      << "\",\"response\":\"" << json_esc(response) << "\"";
    if (!meta.domain.empty())    o << ",\"domain\":\""    << json_esc(meta.domain)    << "\"";
    if (!meta.task_type.empty()) o << ",\"task_type\":\"" << json_esc(meta.task_type) << "\"";
    if (!meta.language.empty())  o << ",\"language\":\""  << json_esc(meta.language)  << "\"";
    if (!meta.split.empty())     o << ",\"split\":\""     << json_esc(meta.split)     << "\"";
    if (meta.quality >= 0.0f)    o << ",\"quality\":"     << meta.quality;
    if (meta.weight != 1.0f)     o << ",\"weight\":"      << meta.weight;
    if (meta.token_count >= 0)   o << ",\"token_count\":" << meta.token_count;
    o << "}";
    return o.str();
}
 ...
 */
inline bool parse_jsonl_sample(const std::string& line, std::string& input,
                                std::string& response, SampleMeta& meta) {
