...
protected:
    ...
    void SetUp() override {
        ...
        model = std::make_shared<EncoderDecoderModel>(
            1000,   // vocab_size
            128,    // d_model
    ...
    void create_test_vocabulary() {
        ...
        const std::vector<std::string> words = {
            ...
            "context", "question", "answer", "document", "retrieval"
        };
    ...
    void add_sample_documents() {
        ...
        doc_store->addDocument("doc2", "Python is a popular programming language for data science.");
...
TEST_F(RAGInferenceTest, ConstructorWithNullModel) {
    EXPECT_THROW({
        RAGInference test_rag(nullptr, doc_store);
    }, std::invalid_argument);
}

TEST_F(RAGInferenceTest, ConstructorWithNullDocumentStore) {
    EXPECT_THROW({
        RAGInference test_rag(model, nullptr);
    }, std::invalid_argument);
}
...
TEST_F(RAGInferenceTest, GenerateWithDocuments) {
    ...
    EXPECT_NO_THROW({
        std::string response = rag->generate("What is machine learning?");
    });
}
