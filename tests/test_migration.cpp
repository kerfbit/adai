...
int main() {
    ...
    std::cout << "  PAD: " << config.pad_token_id 
              << " (expected: " << adai::SpecialTokenIDs::PAD << ")" << std::endl;
    std::cout << "  UNK: " << config.unk_token_id 
              << " (expected: " << adai::SpecialTokenIDs::UNK << ")" << std::endl;
    std::cout << "  BOS: " << config.bos_token_id 
              << " (expected: " << adai::SpecialTokenIDs::BOS << ")" << std::endl;
    std::cout << "  EOS: " << config.eos_token_id 
              << " (expected: " << adai::SpecialTokenIDs::EOS << ")" << std::endl;
    ...
    std::cout << "  is_special_token(2): " 
              << (adai::is_special_token(2, token_config) ? "true" : "false") 
              << " (expected: true)" << std::endl;
    std::cout << "  is_stop_token(3): " 
              << (adai::is_stop_token(3, token_config) ? "true" : "false") 
              << " (expected: true)" << std::endl;
    std::cout << "  get_special_token_string(2): " 
              << adai::get_special_token_string(2, token_config) 
              << " (expected: <bos>)" << std::endl;
