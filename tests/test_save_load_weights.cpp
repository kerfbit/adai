...
bool test_layernorm_save_load() {
    ...
    for (int i = 0; i < 10; i++) {
        if (std::abs(gamma(0, i) - loaded_gamma(0, i)) > 1e-6f) {
            std::cout << "❌ Gamma mismatch at " << i << ": " << gamma(0, i) << " vs " << loaded_gamma(0, i) << std::endl;
            success = false;
        }
        if (std::abs(beta(0, i) - loaded_beta(0, i)) > 1e-6f) {
            std::cout << "❌ Beta mismatch at " << i << ": " << beta(0, i) << " vs " << loaded_beta(0, i) << std::endl;
            success = false;
        }
    }
    ...
    for (int i = 0; i < 10; i++) {
        for (int j = 0; j < 10; j++) {
            if (std::abs(emb1(i, j) - emb2(i, j)) > 1e-6f) {
                std::cout << "❌ Embedding mismatch at (" << i << "," << j << "): " 
                          << emb1(i, j) << " vs " << emb2(i, j) << std::endl;
                success = false;
                goto done;
            }
        }
    ...
    for (int i = 0; i < 5; i++) {
        for (int j = 0; j < 100; j++) {
            if (std::abs(out1(i, j) - out2(i, j)) > 1e-4f) {
                std::cout << "❌ Output mismatch at (" << i << "," << j << "): " 
                          << out1(i, j) << " vs " << out2(i, j) << std::endl;
                success = false;
                goto done2;
            }
        }
